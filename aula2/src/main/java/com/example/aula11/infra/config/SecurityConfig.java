...
public class SecurityConfig {
    ...
    private String pub; // chave publica

    @Value("$jwt.private.key")
    private String priv; // chave privada
    ...
    JwtEncoder jwtEncoder() throws ParseException, JOSEException {
        JWK jwk = new RSAKey.Builder(RSAKey.parse(this.pub).toRSAPublicKey()).privateKey(RSAKey.parse(this.priv).toRSAPrivateKey()).build(); // cria um gerenciador de chaves no java, esse recebe a chave publica e privada
    ...
    JwtDecoder jwtDecoder() throws ParseException, JOSEException {
        return NimbusJwtDecoder.withPublicKey(RSAKey.parse(this.pub).toRSAPublicKey()).build();
    }
